exports.BackendList = [
  "Backend/정적-IP-주소-할당-방식과-동적-IP-주소-할당-방식의-차이점.md",
  "Backend/JWT-특징과-주의-사항.md",
  "Backend/단일-장애-지점.md",
  "Backend/무중단-배포.md",
  "Backend/전략-패턴.md",
  "Backend/분산-환경에서-Redis를-활용한-잠금.md",
  "Backend/테스트-더블.md",
  "Backend/JVM에서-GC-객체를-판단하는-기준.md",
  "Backend/Graceful-Shutdown이-필요한-이유.md",
  "Backend/트랜잭셔널-아웃박스-패턴.md",
  "Backend/CQRS-패턴.md",
  "Backend/의존성-주입.md",
  "Backend/스프링-트랜잭션-AOP의-동작-흐름.md",
  "Backend/동시성-문제-중-경쟁-상태를-해결하려면.md",
  "Backend/가상화.md",
  "Backend/RDB에서-페이징-쿼리의-필요성.md",
  "Backend/CDN.md",
];

exports.DomainDrivenArchitectureList = [
  "DomainDrivenArchitecture/1.-도메인-모델-시작하기.md",
  "DomainDrivenArchitecture/2.-아키텍처-개요.md",
  "DomainDrivenArchitecture/3.-애그리거트.md",
  "DomainDrivenArchitecture/4.-리포지터리와-모델-구현.md",
  "DomainDrivenArchitecture/5.-스프링-데이터-JPA를-이용한-조회-기능.md",
  "DomainDrivenArchitecture/6.-응용-서비스와-표현-영역.md",
  "DomainDrivenArchitecture/7.-도메인-서비스.md",
  "DomainDrivenArchitecture/8.-애그리거트-트랜잭션-관리.md",
  "DomainDrivenArchitecture/9.-도메인-모델과-경계.md",
];

exports.JavaList = [
  "Java/직렬화와-역직렬화.md",
  "Java/Call-by-value와-Call-by-reference.md",
  "Java/String.md",
  "Java/JVM과-컴파일-과정.md",
  "Java/Object.md",
  "Java/Primitive-type과-Reference-Type.md",
  "Java/자바에서-클래스-정보-알아내는-법.md",
  "Java/String-타입-캐스팅과-String.valueOf()-차이.md",
  "Java/자바-프로그램이-실행되는-흐름.md",
  "Java/JCF-자료구조의-초기-용량을-지정하면-좋은-점.md",
  "Java/제네릭의-공변,-반공변,-무공변.md",
  "Java/try-with-resources.md",
];

exports.OperatingSystemList = [
  "OperatingSystem/연속-메모리-할당-기법.md",
  "OperatingSystem/가상메모리,-페이지테이블,-페이지폴트,-스레싱.md",
  "OperatingSystem/페이지-교체-알고리즘.md",
  "OperatingSystem/프로세스와-스레드의-차이.md",
  "OperatingSystem/PCB와-컨텍스트-스위칭.md",
  "OperatingSystem/프로세스의-메모리-구조.md",
  "OperatingSystem/페이지-히트,-페이지-미스.md",
  "OperatingSystem/프로그램의-컴파일-과정.md",
];

exports.PSList = ["PS/펜윅트리.md"];

exports.KubernetesList = [
  "Kubernetes/1.-포드,-레플리카-셋,-디플로이먼트,-서비스.md",
  "Kubernetes/2.-네임스페이스,-컨피그맵,-시크릿,-인그레스.md",
  "Kubernetes/3.-pv와-pvc.md",
  "Kubernetes/4.-ServiceAccount와-RBAC.md",
  "Kubernetes/5.-커스텀-리소스,-컨트롤러,-잡,-데몬셋,-스테이트풀셋.md",
];
